...
const {userProfile , addUser, removeUser } = useAuthStore();
 ...
 const login = useGoogleLogin({
  ...
  return (
    <div className="flex justify-between items-center bg-slate-50 h-auto p-3 fixed top-0 w-full z-40 border-b border-slate-200">
    ...
    { userProfile && (
      ...
      >
        <button className="px-2 py-1 md:py-[1px]  bg-indigo-500 hover:bg-indigo-600 text-white rounded-md  text-md sm:text-lg flex justify-center items-center gap-2"> 
         <span className="hidden md:block">Create</span>  <IoCreateOutline/>
        </button>
      </Link>
    )}
      ...
      {
        userProfile ? (
          <Link to={`profile/${userProfile.sub}`}>
            <div >
              <img src={userProfile.picture} width={35} className='rounded-full'  alt="profile" />
            </div> 
            </Link>
        ) : (
          <div onClick={login}>
